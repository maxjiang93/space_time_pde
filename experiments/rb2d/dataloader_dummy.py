"""RB2 Experiment Dataloader (dummy, for debugging io performance impact)"""
import os
import torch
...
class RB2DataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        space_time_crop_hres = np.random.rand(4, self.nt_hres, self.nz_hres, self.nx_hres)
        space_time_crop_lres = np.random.rand(4, self.nt_lres, self.nz_lres, self.nx_lres)
        point_coord = np.random.rand(self.n_samp_pts_per_crop, 3)
        point_value = np.random.rand(self.n_samp_pts_per_crop, 4)
