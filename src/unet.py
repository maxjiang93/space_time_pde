...
class UNet(nn.Module):
    ...
    def __init__(self, out_channels, in_channels=4, depth=5, start_filts=64, up_mode='transpose', merging_mode='concat'):
        ...
        if self.up_mode == 'upsample' and self.merging_mode == 'add':
            raise ValueError("up_mode \"upsample\" is incompatible " "with merging_mode \"add\"")
        ...
        for i in range(depth):
            ...
            if i != depth-1:
                ...
                ins = outs
                outs = ins // 2
                up_conv = UpConv(ins, outs, up_mode=up_mode, merging_mode="final")
                self.up_convolutions.append(up_conv)
...
if __name__ == "__main__":
    ...
    print("high resolution output image shape:", output.shape)
...
## >> low resolution input image shape: torch.Size([16, 8, 64, 64])
## >> high resolution output image shape: torch.Size([16, 32, 128, 128])
